...
function LiveMatchAlert({ tournament }) {
  ...
  const scorecards = (tournament.scorecards || []).filter(score => score.fixture_id === fixture.id && score.status !== 'abandoned');
  const getTeamName = id => teams.find(team => team.id === id)?.name || id || 'TBD';
  ...
  const statusLabel = score => score.status === 'pending' ? 'Upcoming' : score.status === 'completed' ? 'Completed' : score.status === 'in_progress' ? 'In Progress' : 'On hold';

  return <div className="glass-card" style={{ marginBottom: '1.5rem', borderColor: 'rgba(16, 185, 129, 0.35)', background: 'rgba(16, 185, 129, 0.08)' }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap', marginBottom: '0.85rem' }}><div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}><Activity size={22} color="var(--accent-secondary)" className="pulse" /><div><strong style={{ display: 'block', color: 'var(--accent-secondary)' }}>Live Match</strong><span style={{ color: 'var(--text-secondary)' }}>{getTeamName(fixture.team1_id)} vs {getTeamName(fixture.team2_id)}</span></div></div><Link to="/schedule" className="btn btn-outline" style={{ whiteSpace: 'nowrap' }}>View Match</Link></div>{scorecards.length ? <div style={{ display: 'grid', gap: '0.5rem' }}>{scorecards.map(score => { const event = events.find(item => item.id === score.event_id); const players = [[score.team1_player1, score.team1_player2].filter(Boolean).join(' & ') || 'TBD', [score.team2_player1, score.team2_player2].filter(Boolean).join(' & ') || 'TBD']; const statusColor = score.status === 'pending' ? '#f59e0b' : score.status === 'completed' ? '#60a5fa' : score.status === 'on_hold' ? 'var(--text-secondary)' : 'var(--accent-secondary)'; return <div key={score.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(120px, 0.9fr) minmax(120px, 1fr) minmax(90px, auto) minmax(90px, auto) minmax(120px, 1fr)', alignItems: 'center', gap: '0.75rem', padding: '0.6rem 0.75rem', background: 'rgba(255,255,255,0.04)', borderRadius: 'var(--radius-md)' }}><strong>{event?.name || 'Event'}</strong><span>{players[0]}</span><strong style={{ whiteSpace: 'nowrap', textAlign: 'center' }}>{scoreText(score)}</strong><strong style={{ whiteSpace: 'nowrap', textAlign: 'center', color: statusColor }}>{statusLabel(score)}</strong><span style={{ textAlign: 'right' }}>{players[1]}</span></div>; })}</div> : <span style={{ color: 'var(--text-secondary)' }}>Live match is being prepared.</span>}</div>;
}
